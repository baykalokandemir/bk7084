...
class Building:
    """
    Procedural building generator with configurable architectural styles.
    
    Creates 3D building meshes from 2D polygon footprints with features including:
    - Floor-by-floor construction with columns and inset windows
    - Window glass with realistic frames and sill/header details
    - Stepped/tiered buildings for tall structures (pyramid effect)
    - Randomized rooftop antennas
    - Two window patterns: vertical stripes or single wide windows per floor
    - UV mapping support for textured facades
    
    The generator produces detailed low-poly geometry suitable for city-scale
    rendering with proper normals, vertex colors, and texture coordinates.
    """
    ...
    def __init__(self, footprint, height, style_params=None):
        ...
        # Style defaults from parameters
        ...
        # Stepped building constants
        self.TIER1_HEIGHT_RATIO = 0.5  # Base tier: 50% of total height
        self.TIER2_HEIGHT_RATIO = 0.3  # Middle tier: 30%
        self.TIER3_HEIGHT_RATIO = 0.2  # Top tier: 20%
        self.TIER2_SCALE = 0.7  # Middle tier footprint scale
        self.TIER3_SCALE = 0.4  # Top tier footprint scale
        
        # Column and wall constants
        self.COLUMN_MARGIN_RATIO = 0.15  # Column width as % of wall length
        self.COLUMN_MARGIN_MAX = 1.0     # Maximum column width in units
        
        # Window stripe constants (for vertical_stripes style)
        self.STRIPE_WINDOW_WIDTH = 0.8
        self.STRIPE_GAP_WIDTH = 0.8
        
        # Antenna constants
        self.ANTENNA_HEIGHT_MIN = 2.0
        self.ANTENNA_HEIGHT_MAX = 8.0
        self.ANTENNA_WIDTH = 0.2
        
        # UV constant for non-textured elements
        self.ZERO_UV = glm.vec2(0, 0)

    def generate(self):
        """
        Generates the complete 3D mesh for the building.
        
        Constructs geometry based on style parameters, creating either a single
        block or a stepped/tiered structure for tall buildings. Optionally adds
        rooftop antenna details.
        
        Returns:
            Shape: Complete building mesh with vertices, normals, colors, UVs, and indices
        """
        shape = Shape()
        ...
        if is_stepped and self.height > 15.0:
            ...
            # 3 Tiers
            h1 = self.height * self.TIER1_HEIGHT_RATIO
            h2 = self.height * self.TIER2_HEIGHT_RATIO
            h3 = self.height * self.TIER3_HEIGHT_RATIO
            
            # Tier 1 (Base)
            self._generate_block(self.footprint, 0, h1, all_vertices, all_normals, all_colors, all_indices, all_uvs)
            
            # Tier 2
            poly2 = self.footprint.scale(self.TIER2_SCALE)
            self._generate_block(poly2, h1, h1+h2, all_vertices, all_normals, all_colors, all_indices, all_uvs)
            
            # Tier 3
            poly3 = self.footprint.scale(self.TIER3_SCALE)
    ...
    def _generate_block(self, poly, y_start, y_end, verts, norms, cols, inds, uvs=None):
        """
        Generate geometry for a single building block (tier).
        
        Creates a complete floor-by-floor structure with:
        - Flat roof (top face)
        - Exterior walls divided into columns and window sections
        - Windows with inset glass, sills, headers, and frames
        
        Args:
            poly: Polygon defining the building footprint
            y_start: Bottom elevation of this block
            y_end: Top elevation of this block
            verts, norms, cols, inds, uvs: Mesh data lists to append to
        """
        ...
        for i in range(n_verts):
            ...
            # Columns
            margin = min(self.COLUMN_MARGIN_MAX, edge_len * self.COLUMN_MARGIN_RATIO)
            ...
            for f in range(num_floors):
                ...
                # Determine window layout pattern for this wall section
                window_style = self.style_params.get("window_style", "single")
                
                window_intervals = [] # List of (start_dist, end_dist) along wall for window placement
                
                if window_style == "vertical_stripes" and wall_len > 2.0:
                    # Create thin vertical windows with gaps (modernist style)
                    # Alternating rows are offset by half-period for visual interest
                    win_w = self.STRIPE_WINDOW_WIDTH
                    gap_w = self.STRIPE_GAP_WIDTH
                    period = win_w + gap_w
                ...
                # Iterate through wall, drawing solid sections and window sections
                curr_dist = 0.0
                
                for (s, e) in window_intervals:
                    # Draw Wall before window (if any gap)
                    # Draw solid wall segment before this window (if gap exists)
                    if s > curr_dist + 0.01:
                    ...
                    # Draw Window
                    # Draw window with sill, header, inset glass, and frame details
                    ...
                    # Inset Window
                    # Create inset window geometry
                    # Window is recessed into wall by inset_depth for realistic shadow
                    ...
                    # Frames
                    self._add_quad(verts, norms, cols, inds, uvs, w_p1, w_p2, i_p2, i_p1, glm.vec3(0, 1, 0), self.color, self.ZERO_UV, self.ZERO_UV, self.ZERO_UV, self.ZERO_UV) # Bot
                    self._add_quad(verts, norms, cols, inds, uvs, i_p4, i_p3, w_p3, w_p4, glm.vec3(0, -1, 0), self.color, self.ZERO_UV, self.ZERO_UV, self.ZERO_UV, self.ZERO_UV) # Top
                    self._add_quad(verts, norms, cols, inds, uvs, w_p4, w_p1, i_p1, i_p4, edge_dir, self.color, self.ZERO_UV, self.ZERO_UV, self.ZERO_UV, self.ZERO_UV) # Left
                    self._add_quad(verts, norms, cols, inds, uvs, w_p2, w_p3, i_p3, i_p2, -edge_dir, self.color, self.ZERO_UV, self.ZERO_UV, self.ZERO_UV, self.ZERO_UV) # Right
    ...
    def _add_antenna(self, pos, y_start, verts, norms, cols, inds, uvs):
        """
        Add a simple vertical antenna pole to the rooftop.
        
        Creates a thin rectangular pole with randomized height.
        
        Args:
            pos: 2D position (centroid) for antenna base
            y_start: Elevation to start the antenna (roof height)
            verts, norms, cols, inds, uvs: Mesh data lists to append to
        """
        # Simple pole
        h = random.uniform(self.ANTENNA_HEIGHT_MIN, self.ANTENNA_HEIGHT_MAX)
        w = self.ANTENNA_WIDTH
        ...
        self._add_quad(verts, norms, cols, inds, uvs, p1, p2, t2, t1, glm.vec3(0, 0, -1), color, self.ZERO_UV, self.ZERO_UV, self.ZERO_UV, self.ZERO_UV)
        self._add_quad(verts, norms, cols, inds, uvs, p2, p3, t3, t2, glm.vec3(1, 0, 0), color, self.ZERO_UV, self.ZERO_UV, self.ZERO_UV, self.ZERO_UV)
        self._add_quad(verts, norms, cols, inds, uvs, p3, p4, t4, t3, glm.vec3(0, 0, 1), color, self.ZERO_UV, self.ZERO_UV, self.ZERO_UV, self.ZERO_UV)
        self._add_quad(verts, norms, cols, inds, uvs, p4, p1, t1, t4, glm.vec3(-1, 0, 0), color, self.ZERO_UV, self.ZERO_UV, self.ZERO_UV, self.ZERO_UV)

    def _add_quad(self, verts, norms, cols, inds, uvs, p1, p2, p3, p4, normal, color, uv1, uv2, uv3, uv4):
        """
        Helper to add a quadrilateral face (two triangles) to mesh data.
        
        Args:
            verts, norms, cols, inds, uvs: Mesh data lists to append to
            p1, p2, p3, p4: Four corner positions (vec3) in winding order
            normal: Face normal vector (vec3)
            color: Vertex color (vec4)
            uv1, uv2, uv3, uv4: UV coordinates (vec2) for each corner
        """
        idx = len(verts)
